info( logger, "FORECAST_CARTOLA::forecast atacantes" )

load( "/data-science/projetos/databol/projeto_cartola/modelo_pontuacao/data/modelo_ataque.RData" )


info( logger, "FORECAST_CARTOLA::filtra atacantes" )
...
dados_atacantes_rodada <- dados_cartola %>%
  filter( posicao == "ata",
          rodada_id == rodada ) %>% 
...
info( logger, "FORECAST_CARTOLA::preparar dados forecast atacantes" )

info( logger, "FORECAST_CARTOLA::clusterizar atacantes" )
...
grupos_cluster <- base_cluster %>%
  ...
  mutate( grupos = predict( cluster_model_ataque, .[-1] ) ) %>%
...
dados_forca_clubes <- dados_atacantes_rodada %>% 
